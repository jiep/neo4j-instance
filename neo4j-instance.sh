...
function createDatabase {
    ...
    if [ ! -d "ports/$lastPort" ]; then
        ...
        if [ $version -ge 3 ]; then
          cat "neo4j-skeleton/${neo4jType}-${currentVersion}/conf/neo4j.conf" | sed -e "s/^#dbms.connector.http.address/dbms.connector.http.address/" | sed -e "s/dbms.connector.http.address=0.0.0.0:7474/dbms.connector.http.address=localhost:$lastPort/" | sed -e "s/dbms.connector.https.enabled=true/dbms.connector.https.enabled=false/" | sed -e "s/dbms.connector.bolt.enabled=true/dbms.connector.bolt.enabled=false/" | sed -e "s/^#dbms.shell.enabled=true/dbms.shell.enabled=false/" | sed -e "s/^#dbms.backup.enabled=true/dbms.backup.enabled=false/" > ports/$lastPort/conf/neo4j.conf
        else
